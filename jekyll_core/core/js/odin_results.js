...
{
    // Odin API Url
    var api_url = 'http://www.firstmedicaljobs.com/odin/api/v1/jobs/?search_query=' + search_query + '&zipcode=' + zip_code + '&source=' + source;
    ...
    $.getJSON(api_url, function(data)
    {
        ...
        if(jobs.length > 0)
        {
            ...
            {
                var ad_tags = [{% for tag in site.google_ad_tag_search %}{'title': '{{ tag.title }}', 'adid': '{{ tag.adid }}'}{% if forloop.last %}{% else %},{% endif %}{% endfor %}];
                var ad_size = 'width:728px; height:90px;';
                if ( $(window).width() <= 728 )
                {
                    ad_tags = [{% for tag in site.google_ad_tag_search_mobile %}{'title': '{{ tag.title }}', 'adid': '{{ tag.adid }}'}{% if forloop.last %}{% else %},{% endif %}{% endfor %}];
                ...
                {
                    ad = '<!-- {{ site.text_url }} Search Results Slot ' + ad_count + ' -->'
